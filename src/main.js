...
<aside class="drawer" id="cartDrawer">
  ...
  <form class="checkout-form hidden" id="checkoutForm">
    ...
    <button class="btn full" type="submit">تأكيد الطلب</button>
    <a class="ghost-btn full hidden" id="waOrder" href="#" target="_blank" rel="noreferrer">⚠️ اضغط هنا وأرسل الطلب عبر واتساب لتأكيده</a>
  </form>
</aside>
...
async function sendOrderEmail(order, items) {
  try {
    ...
    await fetch(`${baseUrl}/functions/v1/order-email`, {
      method: "POST",
      headers: {
      ...
      body: JSON.stringify({
        order,
        items,
        shop: settings.store_name || "متجر ويام"
      })
    });
  } catch (error) {
...
async function submitOrder(fd) {
  ...
  for (const i of cart) {
    ...
    if (!p) continue;
    const nextStock = Math.max(0, Number(p.stock || 0) - Number(i.quantity));
    const upd = await supabase.from("products").update({ stock: nextStock, sold_out: nextStock <= 0 ? true : p.sold_out }).eq("id", i.id);
    if (upd.error) throw upd.error;
  }
  const w = sanitizeWhatsapp(settings.whatsapp_number);
  if (w) {
    els.waOrder.href = `https://wa.me/${w}?text=${buildWAOrder(payload, items)}`;
    els.waOrder.textContent = "⚠️ اضغط هنا وأرسل الطلب عبر واتساب لتأكيده";
    els.waOrder.classList.remove("hidden");
  }
  await sendOrderEmail(payload, items);
  cart = [];
  saveCart();
  renderCart();
  closeDrawer();
  await loadProducts();
  showToast(els.toast, "تم حفظ الطلب ✅ الرجاء تأكيده عبر واتساب");
}
...
els.checkoutForm.onsubmit = async (e) => {
  ...
  try {
    ...
    console.error(err);
    showToast(els.toast, "فشل حفظ الطلب");
  }
};
(async function init(){
  applySettings();
